...
export class AuthController {
  ...
  @ApiBody({
    ...
    examples: {
      admin: {
        ...
        value:
          '{"email":"admin@example.com","password":"password123","phoneNumber":"1234567890","name":"John Doe","role":"admin","image":"admin.jpg"}',
      },
      candidate: {
        summary: 'Candidate credentials',
        value:
          '{"email":"user@example.com","password":"mySecret456","phoneNumber":"0987654321","name":"Jane Smith","role":"user","image":"candidate.jpg"}',
      },
      employer: {
        summary: 'Employer credentials',
        value:
          '{"email":"employer@example.com","password":"mySecret456","phoneNumber":"0987654323","name":"Jane Smith","role":"employer","image":"employer.jpg"}',
      },
    },
  })
  ...
  @ApiBody({
    ...
    examples: {
      example1: {
        ...
        value:
          '{"newPassword":"newPassword123", "newEmail":"newUser1@example.com"}',
      },
      example2: {
        summary: 'Another example new password and email',
        value:
          '{"newPassword":"anotherNewPass456", "newEmail":"newUser2@example.com"}',
      },
    },
  })
  async changePasswordAndEmail(
    @Request() req: ExpressRequest,
    @Body('newPassword') newPassword: string,
    @Body('newEmail') newEmail: string,
  ) {
  ...
  async changeInstagramId(
    @Request() req: ExpressRequest,
    @Body('newInstagramId') newInstagramId: string,
  ) {
  ...
  async changeFacebookId(
    @Request() req: ExpressRequest,
    @Body('newFacebookId') newFacebookId: string,
  ) {
  ...
  async changeYoutubeId(
    @Request() req: ExpressRequest,
    @Body('newYoutubeId') newYoutubeId: string,
  ) {
  ...
  async changeLinkedinId(
    @Request() req: ExpressRequest,
    @Body('newLinkedinId') newLinkedinId: string,
  ) {
  ...
  async changeGithubId(
    @Request() req: ExpressRequest,
    @Body('newGithubId') newGithubId: string,
  ) {
  ...
  @ApiBody({
    type: Object,
    description: 'Text to be verified',
    examples: {
      example1: {
        summary: 'Example text',
        value:
          '{"password":"password", "hashedPassword":"$2b$10$21VYtLaJLaDrAAyhrCL71OKplktSfekrlihZ5GJNYfPkkC3HzycjG"}',
      },
      example2: {
        summary: 'Another example text',
        value:
          '{"password":"mySecret123", "hashedPassword":"$2b$10$Lc7GpNKD1aPNPPdIbx7lCu16Hoccj5qpNbm5cXas503a0bs/HLwWm"}',
      },
    },
  })
  ...
  async verifyPassword(@Body() body: any) {
    return {
      varified: await this.authService.verifyPassword(
        body.password,
        body.hashedPassword,
      ),
    };
