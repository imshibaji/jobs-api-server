...
export class AuthService {
  ...
  async signUp(user: SignUpDto): Promise<JwtToken> {
    ...
    const phoneNumberExists = await this.usersService.findByPhoneNumber(
      user.phoneNumber!,
    );
    if (userExists) {
  ...
  async signIn(email: string, pass: string): Promise<JwtToken> {
    try {
      ...
      if (
        user &&
        user.email === email &&
        !(await verifyPassword(pass, user.password))
      ) {
        throw new UnauthorizedException();
      }
      const payload = {
  ...
  async forgetPassword(email: string): Promise<String> {
    ...
    await sendEmail(
      email,
      'Reset Password',
      process.env.APP_URL + `/reset-password/?id=${user.id?.toString()}`,
    );
    return 'success';
  }
  ...
  async changeLinkedinId(
    request: Request,
    newLinkedinId: string,
  ): Promise<UpdateResult> {
    const user = (request as any).user as JwtPayload;
    const userId = user?.sub as number;
    return await this.usersService.update(userId, {
      linkedinId: newLinkedinId,
      updatedAt: new Date(),
    });
  }
