...
export class AuthResolver {
    ...
    async login(@Args('email') email: string, @Args('password') password: string): Promise<JwtToken> {
    ...
    @Mutation(() => String)
    async resetPassword(@Args('userId') userId: string, @Args('newPassword') newPassword: string): Promise<UpdateResult> {
    ...
    @Mutation(() => Boolean)
    async verifyPassword(@Args('password') password: string, @Args('hashedPassword') hashedPassword: string): Promise<boolean> {
