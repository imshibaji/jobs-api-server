...
export class ApplicantsResolver {
    ...
    @Mutation(() => Applicant)
    async createApplicant(@Args('applicant') createApplicant: CreateApplicantDto): Promise<Applicant> {
        return await this.applicantsService.create(createApplicant) as Applicant;
    }

    @Mutation(() => Applicant, { nullable: true })
    async updateApplicant(@Args('id') id: number, @Args('applicant') updateApplicant: UpdateApplicantInput): Promise<Applicant | null> {
